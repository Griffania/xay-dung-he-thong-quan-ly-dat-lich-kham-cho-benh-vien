...
export class JwtStrategy extends PassportStrategy(Strategy, 'jwt') {
  constructor(configService: ConfigService) {
    super({
      ...
      secretOrKey:// Xác minh chữ ký bằng khóa Access Secret, Đọc từ file environment (.env)
        configService.get<string>('JWT_ACCESS_SECRET') ||
        'default_access_secret_123',
    });
  }
   // Hàm chạy tự động sau khi Passport giải mã và kiểm tra token hợp lệ thành công
  validate(payload: JwtPayload) {// Trả về thông tin nào thì req.user ở Controller sẽ nhận được thông tin đó
