...
// Hàm giải mã JWT payload an toàn ở tầng Edge Runtime
function decodeJwtPayload(token: string) {
  try {
    ...
    const jsonPayload = decodeURIComponent(
      atob(base64)//ra binary string mỗi ký tự 1 byte
        .split('')//tách thành array từng ký tự từng byte
        .map((c) => '%' + ('00' + c.charCodeAt(0).toString(16)).slice(-2)) //chuyển đổi byte thành dạng 2 chữ số giống định dạng của url encoded
        .join('')//nói lại thành 1 chuổi 
    );
    return JSON.parse(jsonPayload); // trả về json string sao khi decode utf 8 -> parse thành dạng object json
  } catch (e) {
    return null;
  }
}
